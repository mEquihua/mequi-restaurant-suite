...
export function canOfferOnlinePayment(mode: PaymentMode) {
  return mode === 'ORDER_AND_PAY';
}
export function requestBillIsPrimary(mode: PaymentMode) {
  return mode === 'REQUEST_BILL';
}

export function bucketBoardOrders<T extends { status: string }>(orders: T[]) {
  return orders.reduce<{ preparing: T[]; ready: T[] }>(
    (buckets, order) => {
