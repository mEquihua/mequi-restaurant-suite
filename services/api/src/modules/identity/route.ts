...
export const DUMMY_PIN_HASH =
  '$argon2id$v=19$m=19456,p=1,t=2$WFOhbphr4YF2f5d5cbVt6Q$ZvSA3wsMNnEL1wWs5MfpYFncxBPY4Jw2joqoLxxa7UI';
...
const staffSchema = {
  type: 'object',
  additionalProperties: false,
  required: ['first_name', 'last_name', 'pin', 'role_ids'],
  properties: {
    first_name: { type: 'string', minLength: 1 },
    last_name: { type: 'string', minLength: 1 },
    pin: { type: 'string', pattern: '^\\d{4,12}$' },
    role_ids: { type: 'array', minItems: 1, items: uuidSchema },
  },
} as const;
...
export const identityRoute: FastifyPluginAsync<IdentityRouteOptions> = async (app, options) => {
  ...
  const withSession = <T>(
    ...
    work: (session: Parameters<typeof requirePermission>[0]) => Promise<T>,
  ...
  app.get('/api/v1/terminals', async (request) =>
    withSession(request, async (actor) => {
      ...
      const terminals = await actor.trx
  ...
  app.post(
    ...
    { schema: { headers: terminalCredentialHeaders, body: pinUnlockSchema } },
    async (request, reply) => {
      ...
      if (!body.staff_id || !body.pin)
        throw new IdentityHttpError(400, 'VALIDATION_ERROR', 'staff_id and pin are required.');
      const outcome = await app.withLocationTransaction(
        presentedTerminal.locationId,
        async (trx) => {
          ...
          if (
            ...
            throw new IdentityHttpError(
              401,
              'TERMINAL_UNAUTHENTICATED',
              'The terminal credential is invalid or inactive.',
            );
          }
          ...
          const [staff, location] = await Promise.all([
            ...
            trx
              ...
              .where('id', '=', terminal.location_id)
              .executeTakeFirst(),
          ]);
          const assignedRoles =
            staff && location && staff.organization_id === location.organization_id
              ? await effectiveRoles(trx, staff.id, staff.organization_id)
              : [];
      ...
      if (outcome.kind === 'invalid-pin') {
        throw new IdentityHttpError(
          401,
          'INVALID_PIN',
          'The staff credential or PIN is invalid.',
          undefined,
  ...
  app.post('/api/v1/staff', { schema: { body: staffSchema } }, async (request, reply) => {
    ...
    const staffResponse = await withSession(request, async (actor) => {
      requirePermission(actor, 'iam.staff.create');
      if (
        !body.first_name?.trim() ||
        !body.last_name?.trim() ||
        !/^\d{4,12}$/.test(body.pin ?? '') ||
        ...
        throw new IdentityHttpError(
          400,
          'VALIDATION_ERROR',
          'first_name, last_name, a 4-12 digit PIN, and at least one role are required.',
        );
      }
      const roles = await actor.trx
        .selectFrom('roles')
        .select('id')
        .where('organization_id', '=', actor.organizationId)
        .where('id', 'in', body.role_ids)
        .execute();
      if (roles.length !== new Set(body.role_ids).size)
        throw new IdentityHttpError(
          400,
          'INVALID_ROLE',
    ...
    return reply.status(201).send(staffResponse);
  });
    ...
    async (request, reply) => {
      ...
      const staffResponse = await withSession(request, async (actor) => {
        ...
        if (body.pin !== undefined) {
          if (!/^\d{4,12}$/.test(body.pin))
            throw new IdentityHttpError(400, 'VALIDATION_ERROR', 'PIN must contain 4-12 digits.');
        ...
        if (body.role_ids !== undefined) {
          if (!Array.isArray(body.role_ids) || body.role_ids.length === 0)
            throw new IdentityHttpError(
              400,
              'VALIDATION_ERROR',
              'role_ids must be a non-empty array.',
            );
          const roles = await actor.trx
            .selectFrom('roles')
            .select('id')
        ...
        if (Object.keys(patch).length === 1 && body.role_ids === undefined)
          throw new IdentityHttpError(
            400,
            'VALIDATION_ERROR',
            'At least one mutable staff field or role_ids is required.',
          );
        ...
        if (!updated) {
          ...
          if (!current)
            throw new IdentityHttpError(
              404,
              'NOT_FOUND',
              'Staff member was not found in this location scope.',
            );
          throw new IdentityHttpError(
            409,
            'OPTIMISTIC_CONCURRENCY_CONFLICT',
            'The resource has been modified since it was last read. Please refresh and try again.',
            { current_version: current.version, current_state: publicStaff(current) },
          );
        }
        if (body.role_ids !== undefined) {
          await actor.trx
            .deleteFrom('staff_roles')
      ...
      return reply.send(staffResponse);
    },
  );
  ...
  app.get('/api/v1/roles', async (request) =>
    withSession(request, async (actor) => {
      ...
      for (const row of rows) {
        ...
        if (row.permission_name && row.scope)
          role.permissions.push({ permission_name: row.permission_name, scope: row.scope });
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const roleResponse = await withSession(request, async (actor) => {
        requirePermission(actor, 'iam.roles.update');
        const role = await actor.trx
          .insertInto('roles')
          .values({
            organization_id: actor.organizationId,
            name: body.name.trim(),
      ...
      return reply.status(201).send(roleResponse);
    },
  );
          ...
          properties: {
            permissions: {
              ...
              items: {
                type: 'object',
                additionalProperties: false,
                required: ['permission_name', 'scope'],
                properties: {
                  permission_name: {
                    type: 'string',
                    pattern: '^[a-z][a-z0-9_]*\\.[a-z][a-z0-9_]*\\.[a-z][a-z0-9_]*$',
                  },
                  scope: { type: 'string', enum: ['organization', 'location'] },
                },
              },
            },
          },
      ...
      return withSession(request, async (actor) => {
        ...
        if (
          ...
          body.permissions.some(
            (permission) =>
              !isPermissionName(permission.permission_name) ||
              !['organization', 'location'].includes(permission.scope),
          )
        ) {
          throw new IdentityHttpError(
            400,
            'VALIDATION_ERROR',
            'Each permission must use <domain>.<resource>.<action> and a valid scope.',
          );
        }
        const role = await actor.trx
          .selectFrom('roles')
        ...
        if (body.permissions.length > 0)
          await actor.trx
            ...
            .values(body.permissions.map((permission) => ({ role_id: role.id, ...permission })))
            .execute();
