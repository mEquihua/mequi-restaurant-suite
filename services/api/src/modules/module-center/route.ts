...
export interface ModuleCenterRouteOptions {
  now?: () => Date;
...
function parseIfMatch(value: string | undefined): number {
  if (!value)
    throw new IdentityHttpError(
      ...
      'If-Match is required for an existing module activation.',
    );
...
export const moduleCenterRoute: FastifyPluginAsync<ModuleCenterRouteOptions> = async (
  app,
  options,
) => {
  ...
  app.get('/api/v1/module-definitions', async (request) =>
    withSession(request, async (actor) => {
      requirePermission(actor, 'module_center.modules.read');
      return {
        data: await actor.trx.selectFrom('module_definitions').selectAll().orderBy('key').execute(),
      };
    }),
  );
  ...
  app.get(
    ...
    async (request) =>
      withSession(request, async (actor) => {
        ...
        requireActorLocation(actor, locationId);
        const definitions = await actor.trx
          .selectFrom('module_definitions')
          .selectAll()
          .orderBy('key')
          .execute();
        const activations = await actor.trx
  ...
  async function loadDefinition(actor: { trx: typeof app.db }, moduleKey: string) {
    const definition = await actor.trx
      .selectFrom('module_definitions')
      .select('key')
      .where('key', '=', moduleKey)
      .executeTakeFirst();
  ...
  async function changeStatus(
    ...
    const activation = await withSession(request, async (actor) => {
      requirePermission(actor, 'module_center.modules.write');
      const { locationId, moduleKey } = request.params as { locationId: string; moduleKey: string };
      requireActorLocation(actor, locationId);
      await loadDefinition(actor, moduleKey);
      const current = await actor.trx
        ...
        .executeTakeFirst();
      if (target === 'PAUSED' && (!current || !canPauseModule(current.status as ModuleStatus)))
        throw new IdentityHttpError(
          409,
          'ILLEGAL_MODULE_STATUS_TRANSITION',
          `Cannot pause a module from ${current?.status ?? 'DISABLED'}.`,
        );
      if (!current) {
        const created = await actor.trx
          .insertInto('module_activations')
          .values({
            location_id: locationId,
            module_key: moduleKey,
            status: target,
            attention_reason: reason ?? null,
          })
          .returningAll()
          .executeTakeFirstOrThrow();
        return publicActivation(created);
      }
      const expected = parseIfMatch(request.headers['if-match']);
      const updated = await actor.trx
        .updateTable('module_activations')
        .set({
          status: target,
          attention_reason: target === 'NEEDS_ATTENTION' ? reason! : null,
      ...
      if (!updated)
        throw new IdentityHttpError(
          ...
          { current_version: current.version, current_state: publicActivation(current) },
        );
      return publicActivation(updated);
    });
  ...
  const commandSchema = {
    params: {
      type: 'object',
      additionalProperties: false,
      required: ['locationId', 'moduleKey'],
      properties: { locationId: uuidSchema, moduleKey: moduleKeySchema },
    },
  ...
  app.post(
    ...
    async (request, reply) => changeStatus(request, reply, 'ACTIVE'),
  );
  app.post(
    ...
    async (request, reply) => changeStatus(request, reply, 'PAUSED'),
  );
  ...
  app.post(
    ...
    async (request, reply) => changeStatus(request, reply, 'DISABLED'),
  );
  app.post(
    ...
    async (request, reply) =>
      changeStatus(
        ...
        reply,
        'NEEDS_ATTENTION',
        (request.body as { reason: string }).reason.trim(),
      ),
  );
};
